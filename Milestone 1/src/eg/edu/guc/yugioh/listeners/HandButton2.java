...
public class HandButton2 extends JButton implements MouseListener{
...
if(card instanceof SpellCard){
	...
	if(x.equals("Activate Spell")){
		if(willTakeMonster((SpellCard)this.card)){
			...
			try{
		GUI.getStartFrame().getPlayer2().activateSpell(((SpellCard)this.card), hisCard);}
			...
			try{
		GUI.getStartFrame().getPlayer2().activateSpell(((SpellCard)this.card), MyCard);}
			catch(DefenseMonsterAttackException | MonsterMultipleAttackException | MultipleMonsterAdditionException | NoMonsterSpaceException | NoSpellSpaceException | WrongPhaseException p){
				new JOptionPane();
				JOptionPane.showMessageDialog(null, p.getMessage());
		...
		//GUI.Update();
				...
				try{
				GUI.getStartFrame().getPlayer2().activateSpell(((SpellCard)this.card), null);}
				catch(DefenseMonsterAttackException | MonsterMultipleAttackException | MultipleMonsterAdditionException | NoMonsterSpaceException | NoSpellSpaceException | WrongPhaseException p){
					JOptionPane.showMessageDialog(null, p.getMessage());
	...
	if(x.equals("Set Spell")){
		try{
		GUI.getStartFrame().getPlayer2().setSpell(((SpellCard)this.card));}
		catch(DefenseMonsterAttackException | MonsterMultipleAttackException | MultipleMonsterAdditionException | NoMonsterSpaceException | NoSpellSpaceException | WrongPhaseException p){
			new JOptionPane();
			JOptionPane.showMessageDialog(null, p.getMessage());
		...
		if(card instanceof MonsterCard){
			...
			if(x.equals("Summon Monster")){
				if(((MonsterCard) this.card).getLevel()<=4){
					try{
					GUI.getStartFrame().getPlayer2().summonMonster((MonsterCard)this.card);}
					catch(DefenseMonsterAttackException | MonsterMultipleAttackException | MultipleMonsterAdditionException | NoMonsterSpaceException | NoSpellSpaceException | WrongPhaseException p){
						new JOptionPane();
						JOptionPane.showMessageDialog(null, p.getMessage());
				...
				Object [] myMonsterArea=arrayofMomsters(GUI.getStartFrame().getPlayer2().getField().getMonstersArea());
				String selected=(String) JOptionPane.showInputDialog(null, "Choose to sacrifice","Monster options",GUI.getStartFrame().getPlayer2().getField().getMonstersArea().size(),null,myMonsterArea,"Summon Monster");
				handButtonOption2.setVisible(true);
				...
				if(((MonsterCard) this.card).getLevel()>=7&&((MonsterCard) this.card).getLevel()<=8&&x1.equals("2")){
					 @SuppressWarnings("unchecked")
					ArrayList<MonsterCard> a = (ArrayList<MonsterCard>) GUI.getStartFrame().getPlayer2().getField().getMonstersArea().clone();
					 a.remove(myCard);
				...
				try{
				GUI.getStartFrame().getPlayer2().summonMonster((MonsterCard) this.card, sacrrifices);}
				catch(DefenseMonsterAttackException | MonsterMultipleAttackException | MultipleMonsterAdditionException | NoMonsterSpaceException | NoSpellSpaceException | WrongPhaseException p){
					new JOptionPane();
					JOptionPane.showMessageDialog(null, p.getMessage());
		...
		if(x.equals("Set Monster")){
			...
			if(((MonsterCard) this.card).getLevel()<=4){
				try{
				GUI.getStartFrame().getPlayer2().setMonster((MonsterCard)this.card);}
				catch(DefenseMonsterAttackException | MonsterMultipleAttackException | MultipleMonsterAdditionException | NoMonsterSpaceException | NoSpellSpaceException | WrongPhaseException p){
					new JOptionPane();
					JOptionPane.showMessageDialog(null, p.getMessage());
			...
			try{
			GUI.getStartFrame().getPlayer2().setMonster((MonsterCard) this.card, sacrrifices);}
			catch(DefenseMonsterAttackException | MonsterMultipleAttackException | MultipleMonsterAdditionException | NoMonsterSpaceException | NoSpellSpaceException | WrongPhaseException p){
				new JOptionPane();
				JOptionPane.showMessageDialog(null, p.getMessage());
...
public void mouseEntered(MouseEvent e) {
	...
	if(card instanceof MonsterCard){
		 x=(MonsterCard)card;
	GUI.getGameFrame().getPlayerPanel2().getInfoPanel2().getCardInfo().setText("<html>"+"Name: "+x.getName()+"<br>"+"ATT: "+x.getAttackPoints()+"DFF: "+x.getDefensePoints()+" LVL: "+x.getLevel()+"</html>");
	}
	else{if(card instanceof SpellCard ){
		GUI.getGameFrame().getPlayerPanel2().getInfoPanel2().getCardInfo().setText("Name: "+card.getName());
