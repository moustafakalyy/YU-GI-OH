...
public class SpellButton1 extends JButton implements MouseListener {
	JFrame x;
SpellCard spellcard;
...
JOptionPane spellButtonOption3;
	...
	public SpellButton1(SpellCard spellcard){
...
public void mouseClicked(MouseEvent e) {
	if(GUI.getStartFrame().getPlayer1().equals(GUI.getGameBoard().getActivePlayer())&&!spellcard.equals(null)){
	...
	if(x.equals("Activate Spell")){
		if(willTakeMonster(this.spellcard)){
			...
			try{
		GUI.getStartFrame().getPlayer1().activateSpell((this.spellcard), hisCard);}
			...
			try{
		GUI.getStartFrame().getPlayer1().activateSpell((this.spellcard), MyCard);}
			catch(DefenseMonsterAttackException | MonsterMultipleAttackException | MultipleMonsterAdditionException | NoMonsterSpaceException | NoSpellSpaceException | WrongPhaseException p){
				new JOptionPane();
				JOptionPane.showMessageDialog(null, p.getMessage());
				...
				try{
				GUI.getStartFrame().getPlayer1().activateSpell((this.spellcard), null);}
				catch(DefenseMonsterAttackException | MonsterMultipleAttackException | MultipleMonsterAdditionException | NoMonsterSpaceException | NoSpellSpaceException | WrongPhaseException p){
					new JOptionPane();
					JOptionPane.showMessageDialog(null, p.getMessage());
...
public void mouseEntered(MouseEvent e) {
	if(spellcard!=null &&GUI.getStartFrame().getPlayer1().equals(GUI.getGameBoard().getActivePlayer()))
	GUI.getGameFrame().getPlayerPanel1().getInfoPanel1().getCardInfo().setText("Name: "+spellcard.getName());
