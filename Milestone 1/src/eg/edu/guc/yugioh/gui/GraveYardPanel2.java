...
public class GraveYardPanel2 extends JPanel {
	BufferedImage img ;
	JLabel GyLabel;
	ArrayList<Card> graveYard;
	public GraveYardPanel2(){
		 GyLabel =new JLabel();
...
public void updategraveYardPanel(){
	this.removeAll();
	 ...
	 this.add(GyLabel);
	 graveYard=GUI.getStartFrame().getPlayer2().getField().getGraveyard();
	 if(!GUI.getStartFrame().getPlayer1().getField().getGraveyard().isEmpty())
		...
		try {if(!graveYard.isEmpty() ){
			...
			img=ImageIO.read(new File(GUI.getStartFrame().getPlayer2().getField().getGraveyard().get(GUI.getStartFrame().getPlayer2().getField().getGraveyard().size()-1).getName()+".png"));
		}
		else{
